...
def tmm(matrix_params, wavelength):
    ...
    while i < len(matrix_params):
        M = np.matmul(make_transfer_matrix(matrix_params[i], k0), M)
        i += 1

    field_vector = np.matmul(M,field_vector)
    # The field vector is now the col vector (alpha,beta), which we can use to
    # find r, t:

    E = field_vector[0][0]
    H = field_vector[1][0]
    H_divided = H/(1j*k0)     # For easier calcs
...
# CREATE A MORE GENERAL TMM METHOD THAT CAN APPLY FOR VARIABLE INCIDENCE ANGLES
# WHY? Because we need to be able to determine reflectance and transmittance over
# a set of different incidence angles to evaulate the integral for emissivity calcs.
# Also could be useful later

""" Same as above, take theta to be the initial angle of incidence, which will
...
def general_tmm(matrix_params, wavelength, theta=0):
    n0 = 1                  # refractive index of vacuum
    k0 = n0*2*pi/wavelength     # WAVELENGTH IN METRES
    ...
    # Use Snell's law to find angle of refraction in first medium
    new_theta = theta
    ...
    while i < len(matrix_params):
        # if i == 0:
        # # Use Snell's law to find angle of refraction in each medium
        # # Does not account for TIR? Ok I think something's wrong here
        #     new_theta = arcsin(n0/matrix_params[i][0]*sin(new_theta))   # case for first matrix
        #     # print(theta)
        #     # print(matrix_params)
        # else:
        #     new_theta = arcsin(matrix_params[i-1][0]/matrix_params[i][0]*sin(new_theta))
        new_theta = arcsin(n0/matrix_params[i][0]*sin(theta))
    ...
    # Note we can use theta here since angle of incidence = angle of outgoing
    # ray by symmetry of structure
